...
"""),
    ...
    toggle([rt("Q5 (Hard): Walk me through the full complexity analysis of kernel SVM — memory, training, and prediction.", bold=True)], [
        ...
        para(rt("Training: Building "), eq(r"\mathbf{K}"), rt(" costs "), eq(r"O(n^2 d)"), rt(". The QP solve over "), eq(r"n"), rt(" variables: generic interior-point "), eq(r"O(n^3)"), rt("; SMO "), eq(r"O(n^2)"), rt(" amortized (each step "), eq(r"O(n)"), rt(", empirically "), eq(r)"), eq(r"O(n)"), rt("–"), eq(r"O(n^2)"), rt(" steps).")),
